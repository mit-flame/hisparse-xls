...
async def test_single_cluster_sparse(dut):
    ...
    mlkwargs = {"hbm_chan": 0, "matrix_fp": "/home/ayana/hisparse-xls/data/spmv1.json", "latency": 1, "num_streams": 2, "addr_sig": "t__unified_addr", "pld_sig": "t__unified_pld", "out_vec_buf_len": 4, "in_vec_buf_len": 4}    
...
@cocotb.test()
async def test_single_cluster_dense(dut):
    tester = ProcTester(dut_entity=dut, clock=dut.clk, reset=dut.rst,
                input_signals=[
                    # ml
                    "t__unified_pld",
    ...
    mlkwargs = {"hbm_chan": 0, "matrix_fp": "/home/ayana/hisparse-xls/data/spmv2.json", "latency": 1, "num_streams": 2, "addr_sig": "t__unified_addr", "pld_sig": "t__unified_pld", "out_vec_buf_len": 4, "in_vec_buf_len": 4}    
    vlkwargs = {"mem": [0, 1, 2, 3, 4, 5, 6, 7], "latency": 1, "num_streams": 2, "addr_sig": "t__hbm_vector_addr", "pld_sig": "t__hbm_vector_payload"}
    vb0kwargs = {"vecbuf_name": "vecbuf0", "latency": 1, "banksize": 4, "addr_sig": "t__unified_addr", "pld_sig": "t__streaming_pld"}
    vb1kwargs = {"vecbuf_name": "vecbuf1", "latency": 1, "banksize": 4, "addr_sig": "t__unified_addr", "pld_sig": "t__streaming_pld"}
    pe0bank = [0]*4
    ...
    for row_part in range(2):
        ...
        for _ in range(2):
            ...
            output.extend(payload)
    print(f"Test took {tester.cycle_count} cycles")
    ref = calculate_reference(matrix_fp="/home/ayana/hisparse-xls/data/spmv2.json", vec=list(range(8)))
    print(f"reference {ref}")
    assert all([ref[i] == output[i] for i in range(len(ref))]), "SpMV output did not match"

...
async def driver_core_read_write_new(dut: SimHandleBase, addr_signal_name: str) -> bool:
    await RisingEdge(dut.clk)
    await ReadOnly()
    if getattr(dut, f"{addr_signal_name}_vld").value == 1 and getattr(dut, f"{addr_signal_name}_rdy").value == 1: # is a transaction allowable?
        return True
...
async def driver_core_read_write_worker_new(dut: SimHandleBase, latency: int, addr_signal_name: str) -> Tuple[bool, int, int, int, int, int]:
...
async def vecbuf_driver(dut, vecbuf_name: str, addr_sig: str, pld_sig: str, latency: int = 2, banksize: int = 4):
    ...
    async def vecbuf_driver_worker():
        write_req, commands, real_addr, write_pld, matrix_pld, row_indx = await driver_core_read_write_worker_new(dut=dut, addr_signal_name=f"{vecbuf_name}_{addr_sig}", latency=latency)
    ...
    while True:
        ok = await driver_core_read_write_new(dut=dut, addr_signal_name=f"{vecbuf_name}_{addr_sig}")
        if ok:
